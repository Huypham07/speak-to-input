...
export function FundsList({ onCreateFund }: FundsListProps = {}) {
  ...
  useEffect(() => {
    // Only handle voice intents when on the funds page
    if (pathname !== "/funds") return;

    if (!extractedIntent || !funds || funds.length === 0) return;

    const { intent_type, parameters } = extractedIntent;

    // Only handle fund operation intents
    if (!["deposit_fund", "withdraw_fund", "delete_fund"].includes(intent_type)) {
      return;
    }
  ...
  }, [extractedIntent, funds, clearIntent, pathname]);
  ...
  return (
    <>
      ...
      <FormDialog
        ...
        className="max-w-md w-[95vw] sm:w-full rounded-xl sm:rounded-2xl">
        <FormDialogHeader className="flex-row items-center gap-4 space-y-0 px-6 pt-6 pb-4 border-b border-border bg-card">
          ...
          <div className="flex-1 min-w-0">
            <FormDialogTitle className="text-xl font-semibold text-foreground mb-1">Xác nhận xóa quỹ</FormDialogTitle>
            <FormDialogDescription className="text-sm text-muted-foreground">
            ...
            <div className="flex items-start gap-3 p-4 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-amber-800 dark:text-amber-200 leading-relaxed">
                  Quỹ còn{" "}
                  <span className="font-semibold">{deleteConfirmDialog.currentAmount.toLocaleString("vi-VN")} VND</span>
        ...
        {/* Footer */}
        <div className="flex gap-2 justify-end px-6 py-4 border-t border-border bg-card/50">
          <Button
            variant="outline"
          ...
          <Button
            ...
            disabled={isDeletingFund}
            className="flex-1 sm:flex-initial bg-destructive text-destructive-foreground hover:bg-destructive/90 focus:ring-destructive focus:ring-offset-2 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
            {isDeletingFund ? (
              <>
      ...
      <FormDialog
        ...
        <div className="flex gap-2 justify-end pt-4">
          <Button variant="outline" onClick={() => setDisambiguationDialog({ open: false, funds: [], action: null })}>
            Hủy
          </Button>
        </div>
      </FormDialog>
